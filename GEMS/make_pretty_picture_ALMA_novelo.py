...
for h2x,fex,almaB_,almaR_,almaboth,txt in ((h2normed,fenormed,almablue,almared,almaboth,"normed_"),): #(h2,fe,ks,""),(h2unsharp,feunsharp,ks,"unsharp_")):
    for downsample,size in ((4,'small'),(1,'large')):
        ...
        fe_blue = np.zeros([shape[0],shape[1],3],dtype='float')
        ...
        yslice,xslice = (slice(int(1800/downsample),int(6500/downsample)),
                         slice(int(2400/downsample),int(7600/downsample)))


        almaboth[np.isnan(almaboth)] = 0.0
        almaboth_rgb = np.zeros([shape[0],shape[1],3],dtype='float')
        almaboth_rgb[:,:,0] = logscale(almaboth[::downsample,::downsample],
                                       xmin=almabluemin, xmax=almabluemax,
                                       logexp=almabluescale, toint=False)
        almaboth_red = almaboth_rgb.copy()
        almaboth_hsv = rgb_to_hsv(almaboth_rgb)
        almaboth_hsv[:,:,0] = 340/360.
        almaboth_rgb = hsv_to_rgb(almaboth_hsv)
        #alma_red = alma
        ...
        redblueorange = almaboth_rgb+h2s_orange+fes_blue
        redblueorange[redblueorange>1] = 1

        redgreenblue = almaboth_red + h2_green + fe_blue
        redgreenblue[redgreenblue>1] = 1

        im = PIL.Image.fromarray((redblueorange*255).astype('uint8')[::-1,:])
        im.save(prefix+'Trapezium_GEMS_mosaic_redblueorange_ALMA_novelo_%s%s.png' % (txt,size))
        im = ImageEnhance.Contrast(im).enhance(1.5)
        im.save(prefix+'Trapezium_GEMS_mosaic_redblueorange_ALMA_novelo_%s%s_contrast.png' % (txt,size))
        im = ImageEnhance.Brightness(im).enhance(1.5)
        im.save(prefix+'Trapezium_GEMS_mosaic_redblueorange_ALMA_novelo_%s%s_contrast_bright.png' % (txt,size))

        im = PIL.Image.fromarray((redgreenblue*255).astype('uint8')[::-1,:])
        im.save(prefix+'Trapezium_GEMS_mosaic_redgreenblue_ALMA_novelo_%s%s.png' % (txt,size))
        im = ImageEnhance.Contrast(im).enhance(1.5)
        im.save(prefix+'Trapezium_GEMS_mosaic_redgreenblue_ALMA_novelo_%s%s_contrast.png' % (txt,size))
        im = ImageEnhance.Brightness(im).enhance(1.5)
        im.save(prefix+'Trapezium_GEMS_mosaic_redgreenblue_ALMA_novelo_%s%s_contrast_bright.png' % (txt,size))


        cropped_im = PIL.Image.fromarray((redblueorange[yslice, xslice]*255).astype('uint8')[::-1,:])
        cropped_im.save(prefix+'cropped_Trapezium_GEMS_mosaic_redblueorange_ALMA_novelo_%s%s.png' % (txt,size))

        print("Downsample: ",downsample," size: ",size," style: ",txt)
        #print "Memory Check (ps): ",get_mem()/1024.**3

        output = prefix+'Trapezium_GEMS_mosaic_redblueorange_ALMA_novelo_%s%s.png' % (txt,size)
        avm.embed(output, output)
        output = prefix+'Trapezium_GEMS_mosaic_redblueorange_ALMA_novelo_%s%s_contrast.png' % (txt,size)
        avm.embed(output, output)
        output = prefix+'Trapezium_GEMS_mosaic_redblueorange_ALMA_novelo_%s%s_contrast_bright.png' % (txt,size)
        avm.embed(output, output)
        ...
        im = (np.array(im1, dtype='float') + (almaboth_rgb)[::-1,:,:]*256)
        im[im>255] = 255
        im = PIL.Image.fromarray(im.astype('uint8'))
        im.save(prefix+'Trapezium_GEMS_mosaic_redblueorange_ALMA_novelo_%s%s_contrast_bright2.png' % (txt,size))

        output = prefix+'Trapezium_GEMS_mosaic_redblueorange_ALMA_novelo_%s%s_contrast_bright2.png' % (txt,size)
        avm.embed(output, output)
        ...
        cropped = (np.array(cropped, dtype='float') + (almaboth_rgb)[yslice,xslice][::-1,:,:]*256)
        cropped[cropped>255] = 255
        cropped_im = PIL.Image.fromarray(cropped.astype('uint8'))
        cropped_im.save(prefix+'cropped_Trapezium_GEMS_mosaic_redblueorange_ALMA_novelo_%s%s_contrast_bright2.png' % (txt,size))

        output = prefix+'cropped_Trapezium_GEMS_mosaic_redblueorange_ALMA_novelo_%s%s_contrast_bright2.png' % (txt,size)
        avm.embed(output, output)
